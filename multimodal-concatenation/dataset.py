...
class GraphDataset(DGLDataset):
    ...
    def __getitem__(self, idx):
        ...
        g.nodes["text_node"].data['features'] = torch.tensor(all_text_vec).float()
        g.nodes["image_node"].data['features'] = self.adaptive_pooling(torch.tensor(all_image_vec).float().unsqueeze(0)).squeeze(0)
