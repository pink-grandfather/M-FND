...
class GraphDataset(DGLDataset):
    def __init__(self, df, root_dir, image_id, text_id, 
        ...
        # 用已有的 imageId(s) 生成 clean_image_id
        self.df['clean_image_id'] = self.df['imageId(s)'].str.split('.').str[0]
    ...
    def __getitem__(self, idx):
        ...
        full_image_path = os.path.join(self.root_dir, self.image_vec_dir, f'{file_name_image}_full_image.npy')
        image_vec_full = np.load(full_image_path) if os.path.exists(full_image_path) else np.zeros((1, 768))

        # # Load node embeddings for objects present in the image
        obj_path = os.path.join(self.root_dir, self.image_vec_dir, f'{file_name_image}.npy')
        image_vec = np.load(obj_path) if os.path.exists(obj_path) else image_vec_full
        ...
        text_path = os.path.join(self.root_dir, self.text_vec_dir, f'{file_name_text}.npy')
        full_text_path = os.path.join(self.root_dir, self.text_vec_dir, f'{file_name_text}_full_text.npy')

        text_vec = np.load(text_path)
        text_vec_full = np.load(full_text_path)
        ...
        adjMat = np.ones((all_vec.shape[0], all_vec.shape[0])).astype(float)
        pos_arr = np.where(adjMat > 0.5)

        ## Create tensor tuple for the dgl object 
        u, v = torch.tensor(pos_arr[0], dtype=torch.int64), torch.tensor(pos_arr[1], dtype=torch.int64)
        ...
        g.ndata['features'] = torch.tensor(all_vec).float()
